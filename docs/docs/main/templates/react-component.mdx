import { TemplateOptions } from '@site/docs/components/templateOptions';
import { Example } from '@site/docs/components/example';

# react-component

:::caution

[Install templates](../intro#installation) to use this template

:::

## Usage

```bash title="Usage"
tps react-component <name>
```

```txt title="Creates"
| - <name>
	| - index.js (optional)
	| - <name>.js
	| - <name>.css (optional)
	| - <name>.test.js (optional)
```

<Example>

```bash
tps react-component Nav
```

Produces:

```txt
| - Nav
	| - index.js
	| - Nav.js
	| - Nav.css
```

</Example>

## Options

<TemplateOptions template="react-component" type="js" />

## Copy

If you like this template but want to change a few things, just use the copy
command. It lets you duplicate the template into your project and customize it
as you want.

```bash
tps copy react-component
```

## Examples

### Generate component

```bash
tps react-component Nav
```

Produces:

```txt
| - Nav
	| - index.js
	| - Nav.js
	| - Nav.css
```

### use options

```bash
tps react-component Nav --css --cssType=less --test
```

Produces:

```txt
| - NaV
	| - index.js
	| - Nav.js
	| - Nav.less
	| - Nav.test.js
```

### Configure your react component

Tired of typing in your same responses every time? Initialize your repo with
templates and you can configure the template to fit your projects needs without
having to answer every prompt.

If you havent already initialize your repo:

```bash
tps init
```

lets say your project always uses a `tsx` extention for its components. You can
add `tsx` to the `extension` answer.

```json title=".tps/.tpsrc"
{
  "react-component": {
    "answers": {
      "extension": "tsx"
    }
  }
}
```

Now templates will no longer prompt you for this question and always create your
component with a `tsx` extension.
