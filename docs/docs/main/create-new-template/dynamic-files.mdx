...
Object containing utility functions to help your template making more pleasent.
We incorporate utiliy functions from
[change-case](https://www.npmjs.com/package/change-case) and
[inflection](https://www.npmjs.com/package/inflection).

To see a full list of functions check out the
...
```text
import React from react;
{{? tps.utils.capitalize(tps.name)}}import "{{= tps.name}}.css";
{{?}}
const {{= tps.name}} = (props) => {
	return (
		<div></div>
