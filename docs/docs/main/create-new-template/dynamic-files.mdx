...
Now inside `{{=tps.name}}.jsx.dot` we can use this def by the file name.
...
Now inside `{{=tps.name}}.jsx.dot` we can use these defs.
...
For a full list, you can refer to the
...
prompts. [Prompts](./promtpts) are a way for template owners to ask users for
additional information about how they want their template created. However
[prompts](./promtpts) aren't covered until a bit later in this guide.
