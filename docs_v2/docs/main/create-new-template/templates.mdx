...
Lets create our first template together. For this example we will be creating a
template that generates a `index.js` file for us that, if ran it will print out
"hello world". Easy right? Lets do this...

First change your cwd into a test directory or directory of choice. Well be
using a directory called `tps-example`.
...
This will create a `hello-world` directory inside of the `tps-example/.tps`
directory. Inside of that directory, will be a `default` folder.
...
your directory should now resemble this folder structure
...
This should create a folder called `firstTemplate` inside of `tps-example` like
so:
