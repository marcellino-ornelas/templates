...
template, as opposed to the settings file which is intended for template
development. The configuration file is named `.tpsrc` and it allows you to
define predefined configurations for each template, altering tps behavior during
rendering. With the ability to have multiple template configurations within this
file, users gain greater control and flexibility when utilizing your template.
This file needs to be in a `.tps` folder in the root of your repo.

## Make a configuration file

To generate a configuration file in the current directory, which should be the
root of the repository you are working in, simply run the following command:
...
check out the [tpsrc API](/docs/api/tpsrc)
...
[Template Options API](/docs/api/template#options)
